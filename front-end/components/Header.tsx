...
const Header: React.FC = () => {
    const { 
        ...
        setIsAddExamModalVisible
    } = useStateContext();
    
    return(
        <header className={` w-full header-h sticky top-0 z-20 flex justify-center border-b transition-colors ${ isDarkMode ? 'bg-zinc-900' : 'bg-gray-400' } `}>
            <div className='container h-full flex items-center px-3'>
                ...
                <button
                    ...
                    onClick={ _e => { setIsAddExamModalVisible((_prevState: boolean) => !_prevState); _e.currentTarget.blur(); } }
                >
                    <FontAwesomeIcon 
                        icon={ faPlusCircle }
